...
def demucs_split_file(input_path: str, output_dir, demucs_model: str, audio_format: str, progress = gr.Progress()):
    ...
    if return_code != 0:
        raise RuntimeError(f"demucs.separate failed with exit code {return_code}")

    if not os.path.exists(demucs_inst_file) or not os.path.exists(demucs_vocal_file):
        raise FileNotFoundError(
            f"Demucs output missing: {demucs_inst_file} / {demucs_vocal_file}"
        )
